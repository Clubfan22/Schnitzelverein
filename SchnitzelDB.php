...
class SchnitzelDB {
	...
	function updateUser(array $user){
		$mysqli = $this->mysqli;
		if (!($stmt = $mysqli->prepare("UPDATE users SET id = ?, username = ?, password = ?, salt = ?, email = ? WHERE id=?"))){
			echo "Prepare failed: (".$mysqli->errno.") ". $mysqli->error;
			return false;
		}
		if (!$stmt->bind_param("issssi", $user['id'], $user['username'], $user['password'], $user['salt'], $user['email'], $user['id'])){
			echo "Binding parameters failed: (".$stmt->errno.") ".$stmt->error;
			return false;
		}
		if (!$stmt->execute()){
